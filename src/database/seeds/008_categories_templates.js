...
exports.seed = async function seed(knex) {
  await knex.transaction(async (trx) => {
    // If you're on MySQL, this prevents FK errors during clean-up.
    // (Harmless on MariaDB; remove if you're on PostgreSQL/SQLite.)
    await trx.raw('SET FOREIGN_KEY_CHECKS = 0');

    // Clear dependent tables first
    await trx('notification_templates').del();
    await trx('course_pricing').del();       // <-- dependent on course_categories
    await trx('course_categories').del();

    // Insert course categories
    await trx('course_categories').insert([
      { id: 1, name: 'คอร์สสนทนาเด็ก', name_en: 'Kids Conversation', description: 'คอร์สสนทนาภาษาอังกฤษสำหรับเด็ก', description_en: 'English conversation courses for children', type: 'conversation', sort_order: 1 },
      { id: 2, name: 'คอร์สสนทนาผู้ใหญ่', name_en: 'Adults Conversation', description: 'คอร์สสนทนาภาษาอังกฤษสำหรับผู้ใหญ่', description_en: 'English conversation courses for adults', type: 'conversation', sort_order: 2 },
      { id: 3, name: 'คอร์ส 4 ทักษะ', name_en: 'English 4 Skills', description: 'คอร์สเรียนภาษาอังกฤษครบ 4 ทักษะ', description_en: 'Comprehensive English 4 skills course', type: 'skills', sort_order: 3 },
      { id: 4, name: 'คอร์สเตรียมสอบ IELTS', name_en: 'IELTS Preparation', description: 'คอร์สเตรียมสอบ IELTS', description_en: 'IELTS test preparation course', type: 'test_prep', sort_order: 4 },
      { id: 5, name: 'คอร์สเตรียมสอบ TOEIC', name_en: 'TOEIC Preparation', description: 'คอร์สเตรียมสอบ TOEIC', description_en: 'TOEIC test preparation course', type: 'test_prep', sort_order: 5 },
      { id: 6, name: 'คอร์สเตรียมสอบ TOEFL', name_en: 'TOEFL Preparation', description: 'คอร์สเตรียมสอบ TOEFL', description_en: 'TOEFL test preparation course', type: 'test_prep', sort_order: 6 },
      { id: 7, name: 'คอร์สภาษาจีนสนทนา', name_en: 'Chinese Conversation', description: 'คอร์สสนทนาภาษาจีน', description_en: 'Chinese conversation course', type: 'language', sort_order: 7 },
      { id: 8, name: 'คอร์สภาษาจีน 4 ทักษะ', name_en: 'Chinese 4 Skills', description: 'คอร์สภาษาจีนครบ 4 ทักษะ', description_en: 'Comprehensive Chinese 4 skills course', type: 'language', sort_order: 8 }
    ]);

    // Insert notification templates
    await trx('notification_templates').insert([
      {
